...
export default function EditarProduto() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/produtos/${params.id}`).then((r) => r.json()).then((data) => {
      setForm({
        sku: data.sku, idImpressora: data.idImpressora, categoriaId: String(data.categoriaId), subCategoria: data.subCategoria,
        nome: data.nome, uniMesa: data.uniMesa, idFilamento: data.idFilamento, pesoUsado: data.pesoUsado, custoFilamento: data.custoFilamento,
        tempoHoras: data.tempoHoras, tempoMinutos: data.tempoMinutos, capacidade: data.capacidade, tampa: data.tampa,
        comprimento: data.comprimento, largura: data.largura, altura: data.altura, custoModelo3D: data.custoModelo3D,
        materiais: data.materiais, qtdMateriais: data.qtdMateriais, custoMaterial: data.custoMaterial,
        protecao: data.protecao, qtdProtecao: data.qtdProtecao, custoProtecao: data.custoProtecao,
        embalagem: data.embalagem, custoEmbalagem: data.custoEmbalagem, acabamento: data.acabamento,
        maoObra: data.maoObra, precoSugerido: data.precoSugerido, imagem: data.imagem || "",
      });
      setCarregando(false);
    });
  }, [params.id]);
  ...
  const custoTotal =
    Math.round((form.custoMaterial + form.custoProtecao + form.custoEmbalagem + custoAcab) * 100) / 100;
    ...
    return (
      <div className="p-8 max-w-5xl">
        <div className="animate-pulse space-y-4">
          ...
          <div className="h-96 bg-zinc-100 dark:bg-zinc-800 rounded-xl" />
        </div>
      </div>
    );
  }
