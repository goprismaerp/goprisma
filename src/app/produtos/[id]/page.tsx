  ...
  useEffect(() => {
    ...
    fetch(`/api/produtos/${params.id}`).then((r) => r.json()).then((data) => {
      setForm({
        ...
        qtdMateriais: data.qtdMateriais,
        custoMaterial: data.custoMaterial,
        protecao: data.protecao,
        qtdProtecao: data.qtdProtecao,
        custoProtecao: data.custoProtecao,
          ...
          <div>
            ...
            {precosCalculados && (
              <p className="text-xs text-zinc-500 mt-1">
                Marketplace: R$ {precosCalculados.precoMkplaces.toFixed(2)} | Markup: {((Number(form.precoSugerido) / custoTotal - 1) * 100).toFixed(0)}%
              </p>
            )}
          </div>
        </div>
